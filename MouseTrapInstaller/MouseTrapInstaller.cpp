...
#define NAME_DRIVER _T("MouseTrap")
...
std::vector<tstring> RegGetFilterList() {
	...
	RegGetValue(HKEY_LOCAL_MACHINE, PATH_MOUSECLASS, NAME_UPPERFILTERS, RRF_RT_REG_MULTI_SZ, NULL, regBuffer, &regBufferSize);
...
int __cdecl _tmain(int argc, _TCHAR* argv[]) {
	...
	if(install) {
		...
		for(size_t idx = 0, cnt = list.size(); idx < cnt; idx++) {
			if(list[idx] == _T("mouclass")) { list.insert(begin(list) + idx, NAME_DRIVER); }
